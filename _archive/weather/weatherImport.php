<?php
/**
 * Выдрано со старого 66, особенно не замарачивался что тут и как
 *
 * @TODO: $url1 поменять на настоящий адрес
 */
...
if (!$setOnlyCurrentWeather) {
    /**
     *
     * Тут погода берется на все 10 дней + текущий
     *
     */
...
for ($i = 0; $i < 20; $i++)
{
    if (isset($array['fact_astro']['c'][$i])) {
        ...
        // Формируем новый массив простых данных
        $weather_quick = array();
        ...
        if ($city_id == $cityId) {
            mysql_connect('external.66.ru', 'meteo', 'chee6Nath');
